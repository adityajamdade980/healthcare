<?php
session_start();
// IMPORTANT: Adjust these paths based on the actual location of your files relative to manage-articles.php
// If manage-articles.php is in public_html/backend/, and db.php/functions.php are also in public_html/backend/
require_once 'db.php';
...
// Check for success/error messages from previous redirects (e.g., after delete/update)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_id']) && isset($_POST['delete_type'])) {
    ...
    header("Location: manage-articles"); // Redirect to clean URL
    exit();
}
    ...
    try {
        ...
        if ($article_type === 'health') {
            ...
            $stmt = $db_conn_update->prepare("UPDATE {$table_to_update} SET
                ...
                WHERE id = ?"); // Ensure 'id' is used here, not article_id

            $stmt->bind_param("sssssisssssssssssi", // Note the 'i' for boolean at 6th position, and trailing 'i' for ID
                $category,
                $title,
                $content,
                $medicine_fields['generic_name'],
                $medicine_fields['brand_names'],
                $medicine_fields['prescription_required'],
                $medicine_fields['available_forms'],
                $medicine_fields['active_ingredients'],
                $medicine_fields['adult_dosage'],
                $medicine_fields['elderly_dosage'],
                $medicine_fields['children_dosage'],
                $medicine_fields['pregnancy_safety'],
                $medicine_fields['breastfeeding_safety'],
                $medicine_fields['dosage_chart'],
                $medicine_fields['how_to_take'],
                $medicine_fields['missed_dose'],
                $medicine_fields['overdose_symptoms'],
...
// --- Fetch All Articles (from both tables) ---
$articles = [];
try {
    ...
    if ($health_articles_result) {
        ...
        throw new Exception("Error fetching health articles: " . $health_db->error);
    }

    // Fetch medicine articles
    // Select all relevant medicine columns, provide NULL for image_path as medicines don't have it in this design
    ...
    if ($medicine_articles_result) {
        ...
        throw new Exception("Error fetching medicine articles: " . $medicine_db->error);
    }
...
<head>
    ...
    <style>
        ...
        @media (max-width: 768px) {
            ...
            table {
                min-width: 100%; /* Allow table to shrink if content can wrap */
            }
            th, td {
                padding: 8px 10px; /* Smaller padding */
                font-size: 0.8em;
            }
            td:nth-child(4) { /* Last Updated column */
                white-space: normal; /* Allow date to wrap if too long */
            }
            .btn {
                padding: 5px 8px;
                font-size: 0.8em;
...
<body>
    <header class="main-header">
        <nav class="navbar">
            ...
            <ul class="nav-links">
                <li><a href="adminpage">Create Article</a></li> <li><a href="manage-articles" class="active">Manage Articles</a></li>
    ...
    <div class="admin-dashboard-container">
        <aside class="admin-sidebar">
            <h2>Admin Menu</h2>
            <ul>
                <li><a href="adminpage">Create Article</a></li>
                <li><a href="manage-articles" class="active">Manage Articles</a></li>
                    ...
                    <tbody>
                        ...
                        <?php else: ?>
                            ...
                            <tr>
                                ...
                                <td>
                                    <a href="manage-articles?edit_id=<?= $article['id'] ?>&edit_type=<?= htmlspecialchars($article['article_type']) ?>" class="btn btn-edit">Edit</a>
                ...
                <form method="POST" enctype="multipart/form-data">
                    ...
                    <div class="conditional-field active" id="editMedicineFields">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" id="edit_generic_name" name="generic_name" value="<?= htmlspecialchars($editArticle['generic_name'] ?? '') ?>">
                        </div>
                        <div class="form-group">
                            <label for="edit_brand_names">Brand Names:</label>
                            <input type="text" id="edit_brand_names" name="brand_names" value="<?= htmlspecialchars($editArticle['brand_names'] ?? '') ?>">
                        </div>
                        <div class="form-group">
                            <label for="edit_prescription_required">
                                <input type="checkbox" id="edit_prescription_required" name="prescription_required" value="Yes"
                                    <?= (($editArticle['prescription_required'] ?? 0) == 1) ? 'checked' : '' ?>>
                                Prescription Required
                            </label>
                        </div>
                        <div class="form-group">
                            <label for="edit_available_forms">Available Forms:</label>
                            <input type="text" id="edit_available_forms" name="available_forms" value="<?= htmlspecialchars($editArticle['available_forms'] ?? '') ?>">
                        </div>
                        <div class="form-group">
                            <label for="edit_active_ingredients">Active Ingredients:</label>
                            <textarea id="edit_active_ingredients" name="active_ingredients" rows="3"><?= htmlspecialchars($editArticle['active_ingredients'] ?? '') ?></textarea>
                        </div>
                        <div class="form-group">
                            <label for="edit_adult_dosage">Adult Dosage:</label>
                            <textarea id="edit_adult_dosage" name="adult_dosage" rows="2"><?= htmlspecialchars($editArticle['adult_dosage'] ?? '') ?></textarea>
                        </div>
                        <div class="form-group">
                            <label for="edit_elderly_dosage">Elderly Dosage:</label>
                            <textarea id="edit_elderly_dosage" name="elderly_dosage" rows="2"><?= htmlspecialchars($editArticle['elderly_dosage'] ?? '') ?></textarea>
                        </div>
                        <div class="form-group">
                            <label for="edit_children_dosage">Children Dosage:</label>
                            <textarea id="edit_children_dosage" name="children_dosage" rows="2"><?= htmlspecialchars($editArticle['children_dosage'] ?? '') ?></textarea>
                        </div>
                        <div class="form-group">
                            <label for="edit_pregnancy_safety">Pregnancy Safety:</label>
                            <textarea id="edit_pregnancy_safety" name="pregnancy_safety" rows="2"><?= htmlspecialchars($editArticle['pregnancy_safety'] ?? '') ?></textarea>
                        </div>
                        <div class="form-group">
                            <label for="edit_breastfeeding_safety">Breastfeeding Safety:</label>
                            <textarea id="edit_breastfeeding_safety" name="breastfeeding_safety" rows="2"><?= htmlspecialchars($editArticle['breastfeeding_safety'] ?? '') ?></textarea>
                        </div>
                        <div class="form-group">
                            <label for="edit_dosage_chart">Dosage Chart:</label>
                            <textarea id="edit_dosage_chart" name="dosage_chart" rows="4"><?= htmlspecialchars($editArticle['dosage_chart'] ?? '') ?></textarea>
                        </div>
                        <div class="form-group">
                            <label for="edit_how_to_take">How to Take:</label>
                            <textarea id="edit_how_to_take" name="how_to_take" rows="3"><?= htmlspecialchars($editArticle['how_to_take'] ?? '') ?></textarea>
                        </div>
                        <div class="form-group">
                            <label for="edit_missed_dose">Missed Dose:</label>
                            <textarea id="edit_missed_dose" name="missed_dose" rows="2"><?= htmlspecialchars($editArticle['missed_dose'] ?? '') ?></textarea>
                        </div>
                        <div class="form-group">
                            <label for="edit_overdose_symptoms">Overdose Symptoms:</label>
                            <textarea id="edit_overdose_symptoms" name="overdose_symptoms" rows="2"><?= htmlspecialchars($editArticle['overdose_symptoms'] ?? '') ?></textarea>
                        </div>
                        <div class="form-group">
                            <label for="edit_side_effects">Side Effects:</label>
                            <textarea id="edit_side_effects" name="side_effects" rows="2"><?= htmlspecialchars($editArticle['side_effects'] ?? '') ?></textarea>
                        </div>
                    </div>
                    <?php endif; ?>
                    ...
                    <div class="form-actions">
                        ...
                        <a href="manage-articles" class="btn btn-cancel">Cancel</a>
                    </div>
                </form>
            </div>
            <?php endif; ?>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            if (editForm) {
                const articleTypeSelect = editForm.querySelector('select[name="article_type"]');
                ...
                function updateEditFormCategories() {
                    const currentType = articleTypeSelect.value;
                    const currentCategoryValue = categorySelect.dataset.currentCategory || ''; // Store original category
                    ...
                    // Toggle visibility of image/medicine fields in edit form
                ...
                // Initialize current category value from PHP for JS to use
                if (categorySelect && <?= json_encode($editArticle ? htmlspecialchars($editArticle['category']) : '') ?>) {
                    categorySelect.dataset.currentCategory = <?= json_encode($editArticle ? htmlspecialchars($editArticle['category']) : '') ?>;
                }

                articleTypeSelect.addEventListener('change', updateEditFormCategories);
                updateEditFormCategories(); // Initial call
            }
        });
    </script>
</body>
